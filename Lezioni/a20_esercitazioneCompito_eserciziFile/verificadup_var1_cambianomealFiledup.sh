#se trova il file duplicato, ne cambia l'estensione in .bak
  ...
  if [ $count -eq 0 ]; then
    ...
    for miofile in $files; do
          if [ -f $secondaCart/$miofile ];then
            ...
            nome_senza_est=$(echo $miofile|sed 's/\..*$//')
            nuovoNome=$nome_senza_est.bak
            mv $primaCart/$miofile $primaCart/$nuovoNome
          fi             
    done
  fi
  let count=($count+1)%2
